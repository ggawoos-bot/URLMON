    ...
    if (req.method === 'GET') {
        try {
            ...
            if (fs.existsSync(sitesFilePath)) {
                ...
                // 사이트 목록이 비어있으면 루트 sites.json에서 다시 로드
                if (!sites.sites || sites.sites.length === 0) {
                    if (fs.existsSync(defaultSitesPath)) {
                        console.log('Sites array is empty, reloading from root sites.json');
                        const defaultData = fs.readFileSync(defaultSitesPath, 'utf8');
                        const defaultSites = JSON.parse(defaultData);
                        fs.writeFileSync(sitesFilePath, JSON.stringify(defaultSites, null, 2), 'utf8');
                        console.log(`Reloaded ${defaultSites.sites.length} sites from default configuration`);
                        res.json(defaultSites);
                    } else {
                        res.json({ sites: [] });
                    }
                } else {
                ...
                // 파일이 없으면 루트 sites.json에서 직접 로드 시도
                if (fs.existsSync(defaultSitesPath)) {
                    console.log('Loading directly from root sites.json');
                    const data = fs.readFileSync(defaultSitesPath, 'utf8');
                    const sites = JSON.parse(data);
                    res.json(sites);
                } else {
